  ...
  async execute(interaction, client) {
    ...
    try {
        ...
        if (interaction.isCommand()) {
            ...
            if (!command) {
                logger.warn(`Команда не найдена: ${interaction.commandName}`, {
                    userId: interaction.user.id,
                    guildId: interaction.guildId
                });
                return;
            ...
            try {
                ...
                logger.info(`Команда выполнена: ${interaction.commandName}`, {
                    userId: interaction.user.id,
                    guildId: interaction.guildId,
        ...
        if (interaction.isButton()) {
            // Проверяем, является ли кнопка частью панели ролей
            if (interaction.customId.endsWith('_panel') || 
                interaction.customId === 'cancel_request_panel') {
                try {
                    await panelRolesCommand.handleButton(interaction);
                    logger.info(`Кнопка панели ролей обработана: ${interaction.customId}`, {
                        userId: interaction.user.id,
                        guildId: interaction.guildId
                    });
                } catch (error) {
                    logger.error(`Ошибка обработки кнопки ${interaction.customId}:`, error);
                    await InteractionHelper.safeReply(interaction, {
            ...
            // Обработка других кнопок (можно добавить другие модули)
            logger.debug(`Неизвестная кнопка: ${interaction.customId}`, {
                userId: interaction.user.id
            });
            return;
        }
        ...
        if (interaction.isStringSelectMenu()) {
            if (interaction.customId === 'faction_select_panel') {
                try {
                    await panelRolesCommand.handleSelectMenu(interaction);
                    logger.info(`Выбор фракции обработан: ${interaction.values[0]}`, {
                        userId: interaction.user.id,
                        guildId: interaction.guildId
                    });
                } catch (error) {
                    logger.error('Ошибка обработки выбора фракции:', error);
                    await InteractionHelper.safeUpdate(interaction, {
                        content: '❌ Произошла ошибка при выборе фракции!',
                        components: []
                    });
                }
                return;
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId.startsWith('role_form_')) {
                try {
                    await panelRolesCommand.handleModalSubmit(interaction);
                    logger.info(`Модальное окно обработано: ${interaction.customId}`, {
                        userId: interaction.user.id,
                        guildId: interaction.guildId
                    });
                } catch (error) {
                    logger.error(`Ошибка обработки модального окна ${interaction.customId}:`, error);
                    await InteractionHelper.safeReply(interaction, {
                        content: '❌ Произошла ошибка при отправке формы!',
                        ephemeral: true
                    });
                }
                return;
        ...
        if (interaction.isAutocomplete()) {
            ...
            if (command && command.autocomplete) {
                try {
                    ...
                    logger.error(`Ошибка автокомплита для ${interaction.commandName}:`, error);
                }
            }
            return;
        }
        ...
        logger.error('Критическая ошибка в обработчике взаимодействий:', error);
        
        if (!interaction.replied && !interaction.deferred) {
            try {
                await interaction.reply({
                    content: '❌ Произошла критическая ошибка! Пожалуйста, попробуйте позже.',
                    ephemeral: true
                });
            } catch (replyError) {
